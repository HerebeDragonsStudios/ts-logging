...
export class LoggerImp implements Logger {
    ...
    protected buildMessage(message: LoggerMessage, logLevel: number, issuer: any = undefined, ...args: any[]){
        ...
        if (issuer)
            message = `[${this.getIssuerName(issuer)}] - ${message}`;
        if (this.logLevel)
    ...
    /**
     * Retrieves a proper string representation for the issuer object (or tries to)
     *
     * @param {any} issuer the Class, function, overall object that is calling the log
     * @protected
     */
    protected getIssuerName(issuer: any = undefined): string | undefined {
        if (!issuer)
            return;
